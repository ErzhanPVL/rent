...
function Rabot() {
   const {products} = useContext(ProductsContext);
    const {data,error,isPending}=useFetch('https://rentback-0v37.onrender.com/api/products')
       ...
       <div className='w-full'>
        ...
        <div className='flex gap-4'>
          ...
          {!openModal &&<div className='lg:w-[819px] w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mb-10 gap-2 justify-center'>
            {isPending && <p className='my-10 text-3xl font-bold text-center w-full'>Загрузка</p>}
            {!isPending && !data && <p className='my-10 text-3xl font-bold text-center w-full'>Нет товаров</p>}
            {products && products.map((item)=>{
              return <Link to={`/amstersam/${item.id}`}>
              ...
              <img src={item.image} className='mb-[68px] w-[200px] h-[100px]  ' alt="" />
              {!oncount && <div>
              ...
              <p className='text-[16px] font-bold text-[#1A1A1A] mb-2'>{item.name}</p>
               <p className='flex items-center gap-4'>
                <span className='text-[24px] text-[#1A1A1A] font-[300]'>{item.price} ₽</span>
                <img src="./stol/answer.svg" alt="" />
               </p>
               <p className='font-normal text-[12px] group-hover:mb-6 mb-9'>{item.stock}₽ со второго дня</p>
